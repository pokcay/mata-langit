...
class Admin::MasterListing::UploadsControllerTest < ActionDispatch::IntegrationTest
  setup do
    @admin    = users(:admin)
    @password = "password"
  end
  ...
  test "unauthenticated users cannot access the uploads index" do
    get admin_master_listing_uploads_path
  ...
  test "index paginates at 25 rows per page" do
    log_in_as(@admin)
    baseline = MasterListingUpload.count
    30.times do |i|
      MasterListingUpload.create!(
    ...
    get admin_master_listing_uploads_path(page: 2)
    props = inertia_props
  ...
  test "index filters by year, month and status" do
    log_in_as(@admin)
    get admin_master_listing_uploads_path(month: 2, status: "completed")
    props = inertia_props
  ...
  test "index searches filename case-insensitively" do
    log_in_as(@admin)
    get admin_master_listing_uploads_path(search: "mar")
    props = inertia_props
    assert_equal 1, props["total"]
    assert_equal "Listing Cost 2026 - MAR.xlsx", props["uploads"].first["filename"]
  end

  test "index sorts by total_cost descending" do
    log_in_as(@admin)
    get admin_master_listing_uploads_path(sort: "total_cost", direction: "desc")
    props = inertia_props
  ...
  test "index sorts by composite period ascending" do
    log_in_as(@admin)
    get admin_master_listing_uploads_path(sort: "period", direction: "asc")
    props = inertia_props
  ...
  test "preview flags a new period as not replacing" do
    log_in_as(@admin)
    post preview_admin_master_listing_uploads_path, params: {
      files_metadata: [ { filename: "Listing Cost 2026 - JUL.xlsx", period_year: 2026, period_month: 7, row_count: 5, total_cost: 999_000 } ]
    }, as: :json
    assert_response :success
  ...
  test "preview flags an existing period as a replacement with old totals" do
    log_in_as(@admin)
    post preview_admin_master_listing_uploads_path, params: {
      files_metadata: [ { filename: "Listing Cost 2026 - FEB.xlsx", period_year: 2026, period_month: 2, row_count: 9, total_cost: 5 } ]
    }, as: :json
    assert_response :success
    result = response.parsed_body.first
  ...
  test "preview marks an identical re-upload as unchanged" do
    log_in_as(@admin)
    post preview_admin_master_listing_uploads_path, params: {
      files_metadata: [ { filename: "Listing Cost 2026 - FEB.xlsx", period_year: 2026, period_month: 2, row_count: 2, total_cost: 6_500_000 } ]
    }, as: :json
    assert_response :success
  ...
  test "create stamps the in-file period and enqueues an import job" do
    log_in_as(@admin)
    path = MasterListingFixture.build(title: "JUNE - 2026")
    file = Rack::Test::UploadedFile.new(
      path, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      true, original_filename: "Listing Cost Renamed.xlsx"
    )

    assert_enqueued_with(job: MasterListingImportJob) do
      assert_difference "MasterListingUpload.count", 1 do
        post admin_master_listing_uploads_path, params: { files: [ file ] }
      end
    end

    assert_response :created
    upload = MasterListingUpload.order(:created_at).last
  ...
  test "cancel marks an in-flight upload as cancelled" do
    log_in_as(@admin)
    upload = master_listing_uploads(:pending)
    patch cancel_admin_master_listing_upload_path(upload)
    assert_response :ok
  ...
  test "cancel leaves a completed upload untouched" do
    log_in_as(@admin)
    upload = master_listing_uploads(:completed)
    patch cancel_admin_master_listing_upload_path(upload)
    assert_response :ok
