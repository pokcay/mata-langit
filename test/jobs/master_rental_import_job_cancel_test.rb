...
class MasterRentalImportJobCancelTest < ActiveSupport::TestCase
  ...
  teardown do
    ids = MasterRentalUpload.where(filename: FILENAMES).pluck(:id)
    MasterRentalCost.where(master_rental_upload_id: ids).delete_all
    MasterRentalUpload.where(id: ids).find_each do |u|
      u.file.purge
      u.destroy
    end
  end
  ...
  test "cancelling mid-import rolls back the session and preserves prior-period data" do
    ...
    MasterRentalImportJob.perform_now(baseline.id)
    baseline_count = MasterRentalCost.for_period(2026, 5).count
    ...
    stub = lambda do |_batch|
      unless flipped
        flipped = true
        Thread.new do
          MasterRentalUpload.where(id: replacement.id).update_all(status: "cancelled")
        ensure
    ...
    MasterRentalCost.stub(:insert_all, stub) do
      MasterRentalImportJob.perform_now(replacement.id)
    end

    assert_equal "cancelled", replacement.reload.status
    # Prior upload's rows fully preserved; the cancelled session wrote nothing.
    assert_equal baseline_count, MasterRentalCost.for_period(2026, 5).count
    assert_equal baseline_count,
                 MasterRentalCost.where(master_rental_upload_id: baseline.id).count
    assert_equal 0,
                 MasterRentalCost.where(master_rental_upload_id: replacement.id).count
  end
