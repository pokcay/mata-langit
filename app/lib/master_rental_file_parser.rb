...
# Parses the monthly "Rental Cost {Year} - {Month}" file (the nationwide snapshot
# of in-store display-fixture rental fees) into DB-ready hashes for
# MasterRentalCost.
#
# The workbook contains a single sheet named "RENTAL":
#   row 1  merged title cell A1 — e.g. "MAY - 2026"  ({MONTH NAME} - {YYYY})  ← period source
#   row 2  header:  NO | REGION | AREA | DIST PARENT | DIST CHILD | OUTLET CODE | OUTLET NAME | RENTAL | COST
#   row 3+ data
#
# Period (year + month) is read from the in-file A1 title cell, never from the
# filename. The "NO" column (A) is a row index and is not stored.
class MasterRentalFileParser
  SHEET_NAME = "RENTAL"
  ...
  # Yields batches of insert_all-ready hashes from the RENTAL sheet.
  def self.each_batch(file_path, upload_id:, period_year:, period_month:, batch_size: 1000)
    require "zip"

    base_attrs = {
      master_rental_upload_id: upload_id,
    ...
    Zip::File.open(file_path) do |zip|
      ...
      raise ArgumentError, "Baris header RENTAL tidak ditemukan." unless letter_map

      region_letter = letter_map.key(:region)
  ...
  def self.resolve_sheet_path(zip)
    ...
    raise ArgumentError, "r:id missing for RENTAL sheet." unless r_id
