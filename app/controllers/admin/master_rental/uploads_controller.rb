...
class Admin::MasterRental::UploadsController < Admin::BaseController
  PER_PAGE = 25

  SORT_COLUMNS = {
    "created_at" => "master_rental_uploads.created_at",
    "period"     => :period,
    "row_count"  => "master_rental_uploads.row_count",
    "total_cost" => "master_rental_uploads.total_cost",
    "status"     => "master_rental_uploads.status"
  }.freeze

  # GET /admin/master-rental/uploads
  def index
    scope = MasterRentalUpload.includes(:user)
    ...
    if col == :period
      scope = scope.order(Arel.sql(
        "master_rental_uploads.period_year #{dir}, master_rental_uploads.period_month #{dir}"
      ))
    else
    ...
    available_years = MasterRentalUpload
      .distinct.pluck(:period_year).compact.sort.reverse

    render inertia: "admin/master_rental/Uploads", props: {
  ...
  # POST /admin/master-rental/uploads/preview
  ...
  def preview
    ...
    results = files_metadata.map do |meta|
      ...
      scope          = MasterRentalCost.for_period(period_year, period_month)
  ...
  # POST /admin/master-rental/uploads
  ...
  def create
    ...
    files.each do |file|
      ...
      period = MasterRentalFileParser.read_period(file.tempfile.path)
      ...
      if upload.save
        MasterRentalImportJob.perform_later(upload.id)
  ...
  # PATCH /admin/master-rental/uploads/:id/cancel
  # Called via raw fetch() — not Inertia's router — so head :ok is fine.
  def cancel
    upload = MasterRentalUpload.find(params[:id])
