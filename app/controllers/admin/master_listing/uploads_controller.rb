...
class Admin::MasterListing::UploadsController < Admin::BaseController
  PER_PAGE = 25

  SORT_COLUMNS = {
    "created_at" => "master_listing_uploads.created_at",
    "period"     => :period,
    "row_count"  => "master_listing_uploads.row_count",
    "total_cost" => "master_listing_uploads.total_cost",
    "status"     => "master_listing_uploads.status"
  }.freeze

  # GET /admin/master-listing/uploads
  def index
    scope = MasterListingUpload.includes(:user)
    ...
    if col == :period
      scope = scope.order(Arel.sql(
        "master_listing_uploads.period_year #{dir}, master_listing_uploads.period_month #{dir}"
      ))
    else
    ...
    available_years = MasterListingUpload
      .distinct.pluck(:period_year).compact.sort.reverse

    render inertia: "admin/master_listing/Uploads", props: {
  ...
  # POST /admin/master-listing/uploads/preview
  # Accepts JSON metadata parsed browser-side. No file upload, no DB writes.
  # Body: { files_metadata: [{ filename, period_year, period_month, row_count, total_cost }] }
  def preview
  ...
  # POST /admin/master-listing/uploads
  ...
  def create
    ...
    files.each do |file|
      ...
      period = MasterListingFileParser.read_period(file.tempfile.path)
      ...
      upload = MasterListingUpload.new(
        user:         Current.user,
      ...
      if upload.save
        MasterListingImportJob.perform_later(upload.id)
  ...
  # PATCH /admin/master-listing/uploads/:id/cancel
  # Called via raw fetch() — not Inertia's router — so head :ok is fine.
  def cancel
    upload = MasterListingUpload.find(params[:id])
